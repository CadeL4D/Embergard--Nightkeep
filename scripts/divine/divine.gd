...
## Compatibility name retained while scenes and content migrate. The value is authoritative in
## DivineLedger; personal Faith lives on VillagerRecord and is never spent by the player.
var faith: float:
	get: return DivineLedger.available
	set(value): DivineLedger.set_available(value)
...
func cast(def: PowerDef, world_pos: Vector2) -> bool:
	...
	if centre == -1:
		return false
	if World.is_blighted(centre):
		Events.notice.emit(tr(&"POWER_CORRUPTED_TARGET"), 1)
		return false
	...
	_apply_faith_reaction(def, world_pos)
	return true


func _apply_faith_reaction(def: PowerDef, world_pos: Vector2) -> void:
	var reach := maxf(float(def.radius), 1.0) * Grid.TILE_SIZE
	var reach_sq := reach * reach
	for villager in Colony.villagers:
		if not is_instance_valid(villager) or not villager.alive \
				or villager.position.distance_squared_to(world_pos) > reach_sq:
			continue
		var profile: VillagerRecord = villager.profile
		profile.faith = clampf(profile.faith + def.villager_faith_delta, 0.0, 100.0)
		profile.panic = clampf(profile.panic + def.villager_panic_delta, 0.0, 100.0)
		profile.confusion = clampf(profile.confusion + def.villager_confusion_delta, 0.0, 100.0)
...
func _cast_buff(def: PowerDef, world_pos: Vector2) -> void:
	...
	var reach_sq := reach * reach
	var nephilim_candidate: Villager = null
	var nephilim_distance: float = INF
	for v in Colony.villagers:
		if not is_instance_valid(v) or not v.alive:
			continue
		if v.position.distance_squared_to(world_pos) > reach_sq:
			continue
		if def.creates_nephilim and v.is_adult() and v.profile.villager_type != &"nephilim":
			var candidate_distance: float = v.position.distance_squared_to(world_pos)
			if candidate_distance < nephilim_distance:
				nephilim_distance = candidate_distance
				nephilim_candidate = v
	...
	if nephilim_candidate != null:
		nephilim_candidate.profile.villager_type = &"nephilim"
		nephilim_candidate.profile.faith = 100.0
		nephilim_candidate.max_health = maxf(nephilim_candidate.max_health, 120.0)
		nephilim_candidate.health = nephilim_candidate.max_health
		Events.notice.emit("%s has ascended as Nephilim" % \
			nephilim_candidate.profile.display_name, 2)
...
func _can_place_construct(def: PowerDef, centre: int) -> bool:
	if not def.construct_role.is_empty():
		if Colony.golem_count() >= Colony.GOLEM_CAP \
			...
			return false
		if def.maintenance_influence > DivineLedger.maximum():
			return false
...
func absorb_essence(amount: int) -> void:
	if amount <= 0:
		return
	DivineLedger.add(float(amount))
	Events.faith_changed.emit(faith)


func can_pay(cost: float) -> bool:
	return DivineLedger.can_spend(cost)


func pay(cost: float) -> bool:
	return DivineLedger.spend(cost)
